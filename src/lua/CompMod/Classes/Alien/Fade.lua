...
function Fade:ModifyCrouchAnimation(crouchAmount)    
    ...
    if GetHasStealthUpgrade(self) then
        maxSpeed = maxSpeed + (kFadeStealthWalkSpeedIncrease * 3 / self:GetVeilLevel())
    end
...
function Fade:GetCrouchSpeedScalar()
    ...
    if GetHasStealthUpgrade(self) then
        ...
        local target = baseSpeed * baseModifier + (kFadeStealthWalkSpeedIncrease * 3 / self:GetVeilLevel())
